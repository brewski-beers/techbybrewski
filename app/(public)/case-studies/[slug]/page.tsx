...
import { getCaseStudyBySlug } from "@/lib/firestore/queries";
...
export default function CaseStudyDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => { getCaseStudyBySlug(slug).then(setItem).finally(() => setLoading(false)); }, [slug]);

  if (loading) return <div className="section container"><div className={`skeleton ${styles.skeleton}`} /></div>;
  if (!item) return <div className="section container"><p className="text-body text-muted">Case study not found. <Link href="/case-studies" className="text-accent">← Back</Link></p></div>;
